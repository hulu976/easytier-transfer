...
import * as ClipboardSync from '../modules/clipboardSync';
    ...
    confirm.require({
        ...
        accept: async () => {
            try {
                ...
                ClipboardSync.removeClipboardSync(instanceId.value!);
            } catch (e) {
                console.error(e);
            }
            emits('update');
...
const editNetwork = async () => {
    ...
    try {
        ...
        // 从旁路通道恢复剪贴板同步配置（核心 NetworkConfig 不含该字段）
        ret.clipboard_sync = ClipboardSync.getClipboardSync(instanceId.value!);
        currentNetworkConfig.value = ret;
