...
// ---- 文件传输（PC 端"发送文件"按钮 + 接收提示）----
const toast = useToast()
const sending = ref(false)

async function sendFileViaPicker() {
  sending.value = true
  try {
    const path = await invoke<string>('pick_and_send_file')
    toast.add({ severity: 'success', summary: t('clipboard_sync.send_file'), detail: path, life: 4000 })
  } catch (e: any) {
    const msg = String(e?.message ?? e ?? '')
    if (msg && !msg.includes('未选择文件') && !msg.includes('No file')) {
      toast.add({ severity: 'warn', summary: '发送失败', detail: msg, life: 5000 })
    }
  } finally {
    sending.value = false
  }
...
onMounted(async () => {
  try {
    unlisten = await listen<string>('easyshare://file-received', (e) => {
      toast.add({ severity: 'info', summary: t('clipboard_sync.send_file'), detail: e.payload, life: 6000 })
    })
  } catch {
    /* 非 Tauri 环境忽略 */
  }
...
// 剪贴板同步配置的便捷访问：保证 curNetwork.clipboard_sync 始终存在，便于 v-model 绑定。
const cs = computed<ClipboardSyncConfig>({
  get: () => {
    if (!curNetwork.value.clipboard_sync) {
      curNetwork.value.clipboard_sync = {
        enabled: false,
        file_transfer: false,
        device_name: '',
        network_name: '',
    ...
    return curNetwork.value.clipboard_sync as ClipboardSyncConfig
  },
  set: (v) => {
    curNetwork.value.clipboard_sync = v
  },
})

// 设备名称直接使用网络名称：每次网络名变化时同步过去，对端用网络名识别来源。
watch(
  () => curNetwork.value.network_name,
  (name) => {
    if (name) cs.value.network_name = name
  },
  { immediate: true },
)

// Android 上剪贴板同步依赖无障碍服务（系统限制：后台应用读不到剪贴板），
// 因此仅在安卓宿主里显示授权引导。Web / 桌面端不显示。
const showAccessibilityHint = ref(false)
const accessibilityGranted = ref(true)

async function refreshAccessibilityState() {
  if (!showAccessibilityHint.value) return
  accessibilityGranted.value = await isAccessibilityEnabled()
}

async function grantAccessibility() {
  await openAccessibilitySettings()
  // 用户从设置页返回后再复查一次（无法监听跨应用返回，用延时兜底）
  setTimeout(refreshAccessibilityState, 1500)
}

onMounted(async () => {
  showAccessibilityHint.value = isAndroidHost()
  await refreshAccessibilityState()
})

watch(() => cs.value.enabled, (enabled) => {
  if (enabled) refreshAccessibilityState()
})
</script>
    ...
    <div class="flex flex-col h-full">
      <div class="flex flex-col">
        <div class="w-full self-center ">
          ...
          <Panel :header="t('clipboard_sync.title')" toggleable collapsed>
            <div class="flex flex-col gap-y-3">
              <p class="text-surface-500 text-sm">{{ t('clipboard_sync.help') }}</p>

              <!-- 文件传输：启用后本 App 出现在系统分享面板，可从其他应用直接分享文件给对端 -->
              <div class="flex items-center">
                <ToggleButton v-model="cs.file_transfer" on-icon="pi pi-check" off-icon="pi pi-times"
                  :on-label="t('off_text')" :off-label="t('on_text')" class="w-48" />
                <label class="ml-3 font-bold">{{ t('clipboard_sync.file_transfer') }}</label>
              </div>
              <p class="text-surface-500 text-sm">{{ t('clipboard_sync.file_transfer_help') }}</p>

              <!-- PC 端"发送文件"：调用原生文件选择框，经 pick_and_send_file 发送 -->
              <div v-if="!isAndroidHost && cs.file_transfer" class="flex items-center gap-3">
                <Button :label="t('clipboard_sync.send_file')" icon="pi pi-send" size="small"
                  :loading="sending" @click="sendFileViaPicker" />
                <span class="text-surface-500 text-sm">{{ t('clipboard_sync.send_file_help') }}</span>
              </div>

              <div class="flex items-center">
                <ToggleButton v-model="cs.enabled" on-icon="pi pi-check" off-icon="pi pi-times"
                  :on-label="t('off_text')" :off-label="t('on_text')" class="w-48" />
                <label class="ml-3 font-bold">{{ t('clipboard_sync.enabled') }}</label>
              </div>

              <div class="flex flex-row gap-x-9 flex-wrap">
                <div class="flex flex-col gap-2 basis-5/12 grow">
                  <label for="cs_device_name">
                    {{ t('clipboard_sync.device_name') }}
                    <span class="pi pi-question-circle ml-1 self-center"
                      v-tooltip="t('clipboard_sync.device_name_help')"></span>
                  </label>
                  <!-- 设备名称直接使用网络名称，无需（也不允许）单独填写 -->
                  <InputText id="cs_device_name" :model-value="curNetwork.network_name" readonly
                    :placeholder="t('clipboard_sync.device_name_placeholder')" />
                </div>
                <div class="flex flex-col gap-2 basis-3/12 grow">
                  <label for="cs_port">
                    {{ t('clipboard_sync.port') }}
                    <span class="pi pi-question-circle ml-1 self-center"
                      v-tooltip="t('clipboard_sync.port_help')"></span>
                  </label>
                  <InputNumber id="cs_port" v-model="cs.port" :format="false" :min="1" :max="65535" fluid
                    :disabled="!cs.enabled" />
                </div>
              </div>

              <div class="flex items-center">
                <Checkbox v-model="cs.sync_images" input-id="cs_sync_images" :binary="true"
                  :disabled="!cs.enabled" />
                <label for="cs_sync_images" class="ml-2">
                  {{ t('clipboard_sync.sync_images') }}
                  <span class="pi pi-question-circle ml-1 self-center"
                    v-tooltip="t('clipboard_sync.sync_images_help')"></span>
                </label>
              </div>

              <!-- 安卓专属：无障碍权限引导。系统限制后台应用读取剪贴板，
                   不授权则同步只能单向接收，无法把本机内容发出去。 -->
              <div v-if="showAccessibilityHint && cs.enabled"
                class="flex flex-col gap-y-2 p-3 rounded border"
                :class="accessibilityGranted ? 'border-green-500' : 'border-orange-500'">
                <div class="flex items-center gap-x-2">
                  <span :class="accessibilityGranted ? 'pi pi-check-circle text-green-500' : 'pi pi-exclamation-triangle text-orange-500'"></span>
                  <span class="font-bold">
                    {{ accessibilityGranted
                      ? t('clipboard_sync.accessibility_granted')
                      : t('clipboard_sync.accessibility_required') }}
                  </span>
                </div>
                <p class="text-surface-500 text-sm">{{ t('clipboard_sync.accessibility_help') }}</p>
                <div v-if="!accessibilityGranted">
                  <Button :label="t('clipboard_sync.accessibility_grant')" icon="pi pi-cog"
                    size="small" @click="grantAccessibility" />
                </div>
              </div>
            </div>
          </Panel>
