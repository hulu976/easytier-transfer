...
export interface NetworkConfig {
  ...
  // EasyShare 剪贴板同步配置（旁路字段，不进入 EasyTier 核心 NetworkConfig）
  clipboard_sync?: ClipboardSyncConfig
}
...
// EasyShare 剪贴板/图片跨设备同步配置。
// 该字段仅由 GUI 管理，通过旁路通道（localStorage / 独立 Tauri 命令）持久化与下发，
// 不会进入 EasyTier 核心的 NetworkConfig，因此不修改 EasyTier 核心代码。
export interface ClipboardSyncConfig {
  // 是否启用剪贴板同步
  enabled: boolean
  // 是否启用文件传输（Android 上会把本 App 注册进系统分享面板）
  file_transfer: boolean
  // 在对端显示的本机设备名称（现改为直接使用网络名称，见 network_name）
  device_name: string
  // 网络名称：设备名称直接用它，对端据此识别来源
  network_name: string
  // 是否同步图片（剪贴板图片内容）
  sync_images: boolean
  // EasyShare 监听端口（基于 EasyTier 虚拟网络 TCP）
  port: number
}
