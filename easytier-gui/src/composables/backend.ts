...
import { Api, NetworkTypes, ClipboardSync } from 'easytier-frontend-lib'
...
export async function runNetworkInstance(cfg: NetworkConfig, save: boolean) {
  // 剪贴板同步配置走旁路通道持久化（不进入 EasyTier 核心 NetworkConfig）
  ClipboardSync.saveClipboardSync(cfg.instance_id, cfg.clipboard_sync ?? ClipboardSync.DEFAULT_CLIPBOARD_SYNC)
  const result = await invoke('run_network_instance', { cfg: NetworkTypes.toBackendNetworkConfig(cfg), save })
  // 网络实例启动后，按需派生 EasyShare 剪贴板同步旁路进程
  if (cfg.clipboard_sync?.enabled) {
    try {
      await invoke('run_clipboard_sync', {
        instanceId: cfg.instance_id,
        virtualIp: cfg.virtual_ipv4 ?? '',
        cfg: cfg.clipboard_sync,
      })
    } catch (e) {
...
export async function deleteNetworkInstance(instanceId: string) {
  try {
    await invoke('stop_clipboard_sync', { instanceId })
  } catch (e) {
    console.error('Failed to stop clipboard sync:', e)
  }
...
export async function updateNetworkConfigState(instanceId: string, disabled: boolean) {
  if (disabled) {
    ...
    // 从旁路通道读取剪贴板同步配置，启用时一并启动（虚拟 IP 未知则绑定 0.0.0.0）
    const cfg = ClipboardSync.getClipboardSync(instanceId)
    if (cfg.enabled) {
      try {
        await invoke('run_clipboard_sync', { instanceId, virtualIp: '', cfg })
      } catch (e) {
        console.error('Failed to start clipboard sync:', e)
      }
    }
