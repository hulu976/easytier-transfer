...
// ===== EasyShare 剪贴板同步旁路进程管理（不修改 EasyTier 核心）=====
// 这些命令派生 / 停止一个独立的 easyshare 旁路进程，用于在网络实例启动后
// 在已连接的对端之间同步剪贴板。配置经前端旁路通道（localStorage）传递，
// 不进入 EasyTier 核心的 NetworkConfig，因此完全不修改核心代码。
// 若 easyshare 旁路二进制未随应用打包，则优雅地 no-op（不影响网络本身运行）。

#[derive(serde::Deserialize)]
struct ClipboardSyncConfig {
    enabled: bool,
    #[serde(default)]
    device_name: String,
    /// 设备名称优先使用网络名称（virtual network name），对端据此识别来源。
    #[serde(default)]
    network_name: String,
    #[serde(default)]
    sync_images: bool,
...
// ===== 桌面端 EasyShare（进程内运行，与移动端统一）=====
// 桌面端不再派生侧车二进制，而是把 EasyShare 作为库直接在本进程内启动 TCP 服务，
// 与 Android 端行为完全一致：文件传输的 send_file 直接调用 api::send_file，
// 无需跨进程 IPC。剪贴板由 arboard 轮询；节点发现复用 INSTANCE_MANAGER 路由表。
#[cfg(not(target_os = "android"))]
mod desktop_clipboard {
    ...
    /// 剪贴板监视是否"在线"（有实例在跑且有在线节点时为真，才真正读取剪贴板）。
    static CLIP_ACTIVE: once_cell::sync::Lazy<Arc<AtomicBool>> =
        once_cell::sync::Lazy::new(|| Arc::new(AtomicBool::new(false)));
    /// 屏幕常亮（桌面端恒为真，保持轮询）。
    static SCREEN_ON: once_cell::sync::Lazy<Arc<AtomicBool>> =
        once_cell::sync::Lazy::new(|| Arc::new(AtomicBool::new(true)));
    /// 服务是否在跑（stop 时置 false，停止广播与节点刷新）。
    static RUNNING: once_cell::sync::Lazy<Arc<AtomicBool>> =
        once_cell::sync::Lazy::new(|| Arc::new(AtomicBool::new(false)));
    /// 文件传输启用状态（供 send_file 校验与前端接口一致）。
    static FILE_TRANSFER_ENABLED: AtomicBool = AtomicBool::new(false);
    /// 节点刷新任务是否启动（全局只需一个）。
    static PEER_REFRESHER_STARTED: AtomicBool = AtomicBool::new(false);
    /// 剪贴板监视任务是否启动（全局只需一个）。
    static CLIP_WATCHER_STARTED: AtomicBool = AtomicBool::new(false);

    /// 从核心路由表提取在线对端虚拟 IPv4（排除本机自身）。
    async fn collect_peer_ips() -> Vec<String> {
        let Some(instance_manager) = INSTANCE_MANAGER.read().await.clone() else {
            return Vec::new();
        };
        let Ok(infos) = instance_manager.collect_network_infos().await else {
            return Vec::new();
        };
        ...
        for (_, info) in infos {
            ...
            for route in info.routes {
                let Some(addr) = route.ipv4_addr.as_ref().and_then(|i| i.address.as_ref()) else {
                    continue;
                };
                if addr.addr == 0 || Some(addr.addr) == my_addr {
                    continue;
                }
                ips.push(std::net::Ipv4Addr::from(addr.addr).to_string());
            }
        }
        ips.sort();
        ips.dedup();
    ...
    /// 周期性把在线节点喂给 EasyShare（替代 easytier-cli route --json）。
    fn ensure_peer_refresher() {
        if PEER_REFRESHER_STARTED.swap(true, Ordering::SeqCst) {
            return;
        }
        let active = CLIP_ACTIVE.clone();
        tauri::async_runtime::spawn(async move {
            ...
            loop {
                ...
                let ips = collect_peer_ips().await;
                easyshare::api::update_peers(ips.clone());
                active.store(!ips.is_empty(), Ordering::SeqCst);
            }
        });
    }

    /// 启动剪贴板轮询：本地剪贴板变化时广播给在线节点。
    fn ensure_clipboard_watcher(dev: String) {
        if CLIP_WATCHER_STARTED.swap(true, Ordering::SeqCst) {
            return;
        }
        let active = CLIP_ACTIVE.clone();
        let screen = SCREEN_ON.clone();
        tauri::async_runtime::spawn(async move {
            let watcher = easyshare::clipboard::ClipboardWatcher::new(dev);
            watcher
                .watch_polling(
                    |frame: easyshare::proto::easyshare::ClipboardSync| {
                        if frame.clip_type == easyshare::proto::CLIP_TYPE_IMAGE {
                            easyshare::api::broadcast_image(frame.data);
                        } else if let Ok(text) = String::from_utf8(frame.data) {
                            easyshare::api::broadcast_text(&text);
                        }
                    },
                    active,
                    screen,
    ...
    #[tauri::command]
    pub async fn run_clipboard_sync(
        app: AppHandle,
        instance_id: String,
        virtual_ip: String,
        cfg: ClipboardSyncConfig,
    ) -> Result<(), String> {
        FILE_TRANSFER_ENABLED.store(cfg.file_transfer, Ordering::SeqCst);
        easyshare::api::set_file_transfer(cfg.file_transfer);

        // 剪贴板同步与文件传输任一开启，都需要 EasyShare 服务在跑
        if !cfg.enabled && !cfg.file_transfer {
        ...
        // 文件接收目录：优先用户"下载/EasyTier"，便于发现；兜底应用数据目录。
        let recv_dir = app
            .path()
            .download_dir()
            .map(|p| p.join("EasyTier"))
            .or_else(|_| app.path().app_data_dir().map(|p| p.join("received")))
            .map(|p| p.to_string_lossy().to_string())
            .unwrap_or_else(|_| "EasyTier".to_string());
        let _ = std::fs::create_dir_all(&recv_dir);

        // 设备名优先使用网络名称（与移动端一致），兜底用配置里的 device_name
        let dev = if cfg.network_name.trim().is_empty() {
            &cfg.device_name
        } else {
            &cfg.network_name
        };

        easyshare::api::start("0.0.0.0", cfg.port, dev, cfg.sync_images, &recv_dir);

        // 注册文件接收回调：落盘后向前端发事件弹通知
        let app2 = app.clone();
        easyshare::api::set_file_received_callback(move |path: String| {
            let _ = app2.emit("easyshare://file-received", path);
        });

        RUNNING.store(true, Ordering::SeqCst);
        ensure_peer_refresher();
        ensure_clipboard_watcher(dev.to_string());

        println!(
            "desktop clipboard+file sync started (port={}, file_transfer={})",
            cfg.port, cfg.file_transfer
        );
        Ok(())
    }

    /// 原生文件选择框选文件并发送（PC 端"发送文件"按钮入口）。
    #[tauri::command]
    pub async fn pick_and_send_file() -> Result<String, String> {
    ...
    /// 把本地文件发给所有在线节点（文件传输功能）。
    #[tauri::command]
    pub async fn send_file(path: String) -> Result<String, String> {
        if !easyshare::api::is_enabled() {
            return Err(
                "剪贴板同步/文件传输未启用：请先在「传输与同步」中开启并运行网络".to_string(),
            );
        }
        if !easyshare::api::file_transfer_enabled() {
            return Err(
                "文件传输未启用：请先在「传输与同步」中开启「启用文件传输」".to_string(),
            );
        }
        easyshare::api::send_file(&path);
        Ok(format!("已发送：{path}"))
    }

    #[tauri::command]
    pub async fn stop_clipboard_sync(instance_id: String) -> Result<(), String> {
        let _ = instance_id;
        RUNNING.store(false, Ordering::SeqCst);
        CLIP_ACTIVE.store(false, Ordering::SeqCst);
        easyshare::api::stop();
    ...
    #[tauri::command]
    pub async fn is_accessibility_enabled() -> Result<bool, String> {
        Ok(true)
    }
}
...
/// 移动端剪贴板同步：EasyShare 以库的形式跑在 App 进程内。
///
/// Android 不允许应用执行外部可执行文件，因此这里不派生侧车，而是：
/// 1. 直接在进程内启动 EasyShare 的 TCP 服务；
/// 2. 起一个后台任务，周期性把 EasyTier 路由表里的在线虚拟 IP 喂给 EasyShare
///    （替代桌面端 `easytier-cli route --json` 的节点发现）；
/// 3. 本机剪贴板变化由 Kotlin 侧无障碍服务捕获后经 JNI 广播出去。
#[cfg(target_os = "android")]
mod mobile_clipboard {
    use super::*;
    use std::sync::atomic::{AtomicBool, Ordering};

    /// 文件传输启用状态（由 run_clipboard_sync 注入），供分享面板入口与 send_file 校验。
    static FILE_TRANSFER_ENABLED: AtomicBool = AtomicBool::new(false);

    /// 路由表刷新任务是否已启动（全局只需一个）。
    static PEER_REFRESHER_STARTED: AtomicBool = AtomicBool::new(false);

    /// 从核心的路由表里提取所有对端虚拟 IPv4（排除本机自身）。
    async fn collect_peer_ips() -> Vec<String> {
        let Some(instance_manager) = INSTANCE_MANAGER.read().await.clone() else {
            return Vec::new();
        };
    ...
    /// 启动路由表刷新任务：每 5 秒同步一次在线节点列表给 EasyShare。
    fn ensure_peer_refresher() {
        if PEER_REFRESHER_STARTED.swap(true, Ordering::SeqCst) {
            return;
        }
        tauri::async_runtime::spawn(async move {
            ...
            loop {
                ...
                let ips = collect_peer_ips().await;
                easyshare::api::update_peers(ips);
            }
        });
    }
    ...
    pub async fn run_clipboard_sync(
        ...
        // 记录文件传输开关（供分享面板入口与 send_file 命令校验）
        FILE_TRANSFER_ENABLED.store(cfg.file_transfer, Ordering::SeqCst);
        easyshare::api::set_file_transfer(cfg.file_transfer);
        ...
        // 虚拟网卡地址可能晚于本调用就绪，统一绑 0.0.0.0 更稳
        let _ = virtual_ip;
        // 设备名优先使用网络名称（virtual network name），对端据此识别来源
        let dev = if cfg.network_name.trim().is_empty() {
            &cfg.device_name
        } else {
            &cfg.network_name
        ...
        println!(
            "clipboard+file sync started for instance {instance_id} (port={}, file_transfer={})",
            cfg.port, cfg.file_transfer
        );
        Ok(())
    }
    ...
    /// 把本地文件发给所有在线节点（文件传输功能）。
    /// 由 Android 系统分享面板入口调用；未启用文件传输时返回友好错误。
    #[tauri::command]
    pub async fn send_file(path: String) -> Result<String, String> {
        if !FILE_TRANSFER_ENABLED.load(Ordering::SeqCst) {
            return Err(
                "文件传输未启用：请先在「传输与同步」中开启「启用文件传输」并运行网络".to_string(),
            );
        }
        easyshare::api::send_file(&path);
        Ok(format!("已发送：{path}"))
    ...
    /// Android 端文件发送由系统分享面板触发，此命令仅占位（前端 PC 按钮调用）。
    #[tauri::command]
    pub async fn pick_and_send_file() -> Result<String, String> {
        Err("Android 端请使用系统分享面板发送文件".to_string())
    }
    ...
    /// 打开系统"无障碍"设置页，引导用户授予后台读取剪贴板的能力。
    #[tauri::command]
    pub async fn open_accessibility_settings() -> Result<(), String> {
        easyshare::android::open_accessibility_settings();
        Ok(())
    }

    /// 查询 EasyShare 的无障碍服务是否已被用户开启。
    #[tauri::command]
    pub async fn is_accessibility_enabled() -> Result<bool, String> {
        Ok(easyshare::android::is_accessibility_enabled())
    }
}
